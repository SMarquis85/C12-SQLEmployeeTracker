    ...
    .prompt([
      {
        ...
        type: "input",
        message:
          "What is the department you would like to remove?",
      },
    ])
    .then((answer) => {
      const query = `SELECT dep_id AS Department_ID,
  INNER JOIN departments ON roles.dept_id = departments.dept_id 
  WHERE ?`;
      connection.query(query, { last_name: answer.depToRemove }, (err, res) => {
        if (err) throw err;
        if (res.length === 0) {
          ...
          inquirer
            .prompt({
              name: "depConfirm",
              type: "number",
              message: "Please enter the department to confirm choice:",
            })
            .then((answer) => {
              const query = "SELECT * FROM Employees WHERE ?";
              connection.query(query, { emp_id: answer.depConfirm }, (err, res) => {
                if (err) throw err;
                let depToDelete = answer.idConfirm;
                const deleteQuery = `DELETE = ${depToDelete}`;
                connection.query(deleteQuery, (err, res) => {
                  if (err) throw err;

                  console.log(chalk.green.bold(`====================================================================================`));
                  console.log(`                  ` + chalk.red.bold(`Department #${depToDelete} has been removed.`));
                  console.log(chalk.green.bold(`====================================================================================`));
